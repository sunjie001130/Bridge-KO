...
# Background-regularization penalty (L_BG) lives alongside this runner so the
# vendored DL package stays untouched (lambda_bg=0 -> no-op, cached AUCs hold).
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from bg_reg import add_background_regularization  # noqa: E402
...
def fit_select_eval(run, Xtr_df, ytr, Xev_df, yev, run_dir, epochs, batch_size,
                    lambda_bg=0.0, margin=0.0):
    """One BRIDGE-KO pass: train full -> FDR select -> retrain -> AUC on (Xev, yev).

    Shared by run_one_seed (eval = held-out test) and tune_bg_reg.py (eval =
    internal validation, then held-out test). The caller is responsible for
    seeding. With lambda_bg<=0 this is identical to the original pipeline:
    add_background_regularization is a no-op, so cached AUCs reproduce.

    Returns (auc, n_selected, selected_features).
    """
    ...
    add_background_regularization(model_full, lambda_bg=lambda_bg, margin=margin)
    ...
    if n_sel == 0:
        return np.nan, 0, selected_features

    # Step 3+4: retrain on selected, AUC on eval set
    x3d_tr_sel, _, _ = build_x3d_from_knockoff(Xtr_df, keep_cols=selected_features)
    x3d_ev_sel, _, _ = build_x3d_from_knockoff(Xev_df, keep_cols=selected_features)
    ...
    add_background_regularization(model_sel, lambda_bg=lambda_bg, margin=margin)
    model_sel.fit(x3d_tr_sel, ytr, epochs=epochs, batch_size=batch_size, verbose=0)
    yprob = model_sel.predict(x3d_ev_sel, verbose=0).reshape(-1)
    return roc_auc_score(yev, yprob), n_sel, selected_features


def run_one_seed(run, seed, Xtr_df, ytr, Xte_df, yte, run_dir, epochs, batch_size,
                 lambda_bg=0.0, margin=0.0):
    set_global_seed(seed)
    auc, n_sel, _ = fit_select_eval(run, Xtr_df, ytr, Xte_df, yte, run_dir,
                                    epochs, batch_size, lambda_bg=lambda_bg, margin=margin)
    return auc, n_sel
...
def main():
    ...
    ap.add_argument("--lambda-bg", type=float, default=0.0,
                    help="strength of L_BG background-regularization (0 = off, reproduces baseline)")
    ap.add_argument("--margin", type=float, default=0.0,
                    help="margin m>=0 in the L_BG hinge")
    args = ap.parse_args()
    ...
    for run in range(args.n_seeds):
        ...
        auc, n = run_one_seed(run, seed, Xtr_df, ytr, Xte_df, yte,
                              ...
                              lambda_bg=args.lambda_bg, margin=args.margin)
        aucs.append(auc); nsel.append(n)
        print(f"[{args.tag}] run {run+1}/{args.n_seeds} seed={seed} "
              f"lambda_bg={args.lambda_bg} margin={args.margin} n_sel={n} auc={auc}", flush=True)
